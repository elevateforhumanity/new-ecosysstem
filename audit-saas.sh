...
table_header
if have_file "vercel.json" || have_dir ".vercel"; then
  row "Vercel config" "$(found "present")"
else
  row "Vercel config" "$(info "not detected")"
fi

if have_file "netlify.toml"; then
  row "Netlify config" "$(info "present")"
fi

if rg -n 'durable' 2>/dev/null | grep -q .; then
  row "Durable references" "$(info "found in repo")"
fi
