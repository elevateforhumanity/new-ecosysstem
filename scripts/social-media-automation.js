...
class SocialMediaAutomation {
  ...
  getContentTemplate(type) {
    const templates = {
      program_highlight: {
        ...
        hashtags: ['#CareerDevelopment', '#WorkforceDevelopment', '#Certification', '#Training'],
        cta: 'Enroll Today'
      },
      success_story: {
        ...
        hashtags: ['#SuccessStory', '#CareerChange', '#Inspiration', '#ElevateForHumanity'],
      ...
      funding_opportunity: {
        ...
        hashtags: ['#FederalFunding', '#WIOA', '#FreeTraining', '#CareerOpportunity'],
      ...
      certification_spotlight: {
        ...
        hashtags: ['#Certification', '#CareerGrowth', '#ProfessionalCertification'],
        cta: 'Get Certified'
      }
    };
  ...
  async postToFacebook(pageConfig, content) {
    ...
    try {
      ...
      console.log(`✅ Posted to Facebook ${pageConfig.name}: ${response.data.id}`);
  ...
  async triggerZapierWebhook(content, results) {
    ...
    try {
      ...
      const response = await axios.post(this.config.zapier.webhookUrl, webhookData);
  ...
  async executeScheduledPost(content) {
    ...
    for (const platform of content.platforms) {
      if (platform === 'facebook_1') {
        results.facebook_1 = await this.postToFacebook(this.config.facebook.page1, content);
      } else if (platform === 'facebook_2') {
        results.facebook_2 = await this.postToFacebook(this.config.facebook.page2, content);
  ...
  generateDailyReport() {
    ...
    const todayPosts = this.postHistory.filter(p => p.timestamp.startsWith(today));
    
    const report = {
      date: today,
      total_posts: todayPosts.length,
      platforms: {
        facebook_1: todayPosts.filter(p => p.results.facebook_1?.status === 'success').length,
        facebook_2: todayPosts.filter(p => p.results.facebook_2?.status === 'success').length,
        youtube: todayPosts.filter(p => p.results.youtube?.status === 'success').length,
        linkedin: todayPosts.filter(p => p.results.linkedin?.status === 'success').length,
        durable_blog: todayPosts.filter(p => p.results.durable_blog?.status === 'success').length
      },
      success_rate: (todayPosts.filter(p => 
        Object.values(p.results).some(r => r.status === 'success')
      ).length / todayPosts.length * 100).toFixed(2) + '%',
      posts: todayPosts
    };
  ...
  savePostHistory() {
    const historyPath = path.join(__dirname, '../.autopilot_out/social-media-history.json');
  ...
  startScheduler() {
    ...
    console.log(`   Schedule: ${this.config.schedule.times.join(', ')} ${this.config.schedule.timezone}`);
    console.log(`   Platforms: Facebook (2), YouTube, LinkedIn, Durable Blog`);
    console.log(`   Zapier: ${this.config.zapier.enabled ? 'Enabled' : 'Disabled'}\n`);
    ...
    dailyContent.forEach(content => {
      ...
      const now = new Date();
      const scheduledTime = new Date(now.getFullYear(), now.getMonth(), now.getDate(), parseInt(hour), parseInt(minute));
      ...
      console.log(`⏰ Scheduled ${content.type} for ${scheduledTime.toLocaleString()}`);
    });
  }
}
