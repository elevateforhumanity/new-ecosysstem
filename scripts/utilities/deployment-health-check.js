...
class DeploymentHealthCheck {
    ...
    async runHealthCheck() {
        ...
        for (const [systemName, system] of Object.entries(this.expectedFeatures)) {
            ...
            if (systemStatus.missingFiles.length > 0) {
                console.log(`   Missing files: ${systemStatus.missingFiles.join(', ')}`);
            }
            if (systemStatus.brokenFeatures.length > 0) {
                console.log(`   Broken features: ${systemStatus.brokenFeatures.join(', ')}`);
            }
        }
    ...
    generateHealthReport(results) {
        ...
        const successRate = Math.round((results.working.length / totalSystems) * 100);
        console.log(`   ${successRate}% of systems fully operational`);
        console.log(`   ${results.working.length}/${totalSystems} systems working correctly`);
