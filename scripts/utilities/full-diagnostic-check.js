...
class FullDiagnosticCheck {
  ...
  async analyzePage(page) {
    ...
    if (
      page.includes('sister-sites') ||
      page.includes('kingdom') ||
      page.includes('serene') ||
      page.includes('urban')
    ) {
  ...
  generateDiagnosticReport() {
    ...
    const workingPages = Object.values(this.results.pageAnalysis).filter(
      (p) => p.loads
    ).length;
    const totalIssues =
    ...
    console.log(
      `   Pages Loading: ${workingPages}/${totalPages} (${Math.round((workingPages / totalPages) * 100)}%)`
    );
    ...
    console.log(
      `   2. Google PageSpeed Insights - Mobile & desktop performance`
    );
