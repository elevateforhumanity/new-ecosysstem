...
async function runChecks() {
  ...
  check('Unified server exists', fs.existsSync('./server/main.ts'), 'server/main.ts file present');
  ...
  check('Compliance router exists', fs.existsSync('./server/routes/compliance.ts'));
  check('Payment router exists', fs.existsSync('./server/routes/payments.ts'));
  check('Database service exists', fs.existsSync('./server/services/database.ts'));
  
  // Documentation checks
  check('README updated', fs.existsSync('./README.md') && fs.readFileSync('./README.md', 'utf8').includes('Production Hardening Checklist'));
  ...
  check('JWT secret placeholder', envExample.includes('JWT_SECRET=') && envExample.includes('minimum_32_characters'));
  ...
  try {
    ...
    check('Legacy server compatibility', fs.existsSync('./simple-server.cjs'), 'simple-server.cjs exists for backward compatibility');
  } catch (error) {
