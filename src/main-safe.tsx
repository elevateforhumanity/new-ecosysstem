...
function BootError({ err }: { err: unknown }) {
  return (
    <main style={{ fontFamily: 'system-ui', padding: 24 }}>
      ...
      <p style={{ marginTop: '1rem', color: 'var(--brand-text-muted)' }}>
        Check import path casing, lazy imports, env usage, router setup.
      </p>
      <button
...
async function start() {
  ...
  if (!rootEl) {
    ...
    document.body.innerHTML =
      '<h1 style="color:red;padding:2rem">FATAL: #root element not found</h1>';
    throw err;
  }
  ...
  try {
    ...
    if (!App) {
      throw new Error(
        'App default export missing - check that App.jsx exports default'
      );
    }
