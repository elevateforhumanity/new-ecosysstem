  ...
  return (
    <AppLayout title="Donate">
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem' }}>
        <header style={{ textAlign: 'center', marginBottom: '3rem', padding: '3rem 2rem', background: 'linear-gradient(135deg, var(--brand-secondary) 0%, var(--brand-secondary) 100%)', borderRadius: '12px', color: 'white' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '3rem' }}>
          <div>
            <form onSubmit={handleDonate} style={{ background: 'white', padding: '2rem', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
              ...
              <div style={{ marginBottom: '2rem' }}>
                <label style={{ display: 'block', marginBottom: '0.75rem', fontWeight: '600', color: 'var(--brand-text)' }}>Donation Frequency</label>
                <div style={{ display: 'flex', gap: '1rem' }}>
                  <button type="button" onClick={() => setFrequency("once")} style={{ flex: 1, padding: '0.75rem', backgroundColor: frequency === "once" ? 'var(--brand-secondary)' : 'white', color: frequency === "once" ? 'white' : 'var(--brand-text)', border: '2px solid var(--brand-secondary)', borderRadius: '6px', fontWeight: '600', cursor: 'pointer' }}>One-Time</button>
                  <button type="button" onClick={() => setFrequency("monthly")} style={{ flex: 1, padding: '0.75rem', backgroundColor: frequency === "monthly" ? 'var(--brand-secondary)' : 'white', color: frequency === "monthly" ? 'white' : 'var(--brand-text)', border: '2px solid var(--brand-secondary)', borderRadius: '6px', fontWeight: '600', cursor: 'pointer' }}>Monthly</button>
                </div>
              </div>
              ...
              <button type="submit" style={{ width: '100%', padding: '1rem', backgroundColor: 'var(--brand-secondary)', color: 'white', border: 'none', borderRadius: '6px', fontSize: '1.125rem', fontWeight: '600', cursor: 'pointer' }}>
          ...
          <div>
            ...
            <div style={{ background: 'var(--brand-surface)', padding: '2rem', borderRadius: '8px' }}>
              <h3 style={{ fontSize: '1.25rem', marginBottom: '1rem', color: 'var(--brand-info)' }}>Other Ways to Give</h3>
              <p style={{ color: 'var(--brand-text)', marginBottom: '1rem' }}>Interested in corporate partnerships or planned giving?</p>
              <Link to="/philanthropy" style={{ display: 'inline-block', padding: '0.75rem 1.5rem', backgroundColor: 'var(--brand-info)', color: 'white', textDecoration: 'none', borderRadius: '6px', fontWeight: '600' }}>Learn More</Link>
            </div>
          </div>
        </div>
      </div>
